$(document).ready(function(){
  ...
  $('#dip_total_fees_paid').focus(function() {
    var dip_total_fees = parseInt($('#dip_college_fees_paid').val()) + parseInt($('#dip_fees_paid').val());
    $('#dip_total_fees_paid').val(dip_total_fees);
});
 ...
 /* if(error_adm_date != '' || error_st_name != '' || error_st_caste != '' || error_st_mobile_no != '')
  ...
  $('#btn_admission_details').click(function(){
    ...
    if($("#dcet").is(":checked")){
      if($.trim($('#dip_adm_order_no').val()).length == 0)
      {
      //alert("Admission order number is required.");
      error_dip_adm_order_no = 'Admission order number is required.';
      $('#error_dip_adm_order_no').text(error_dip_adm_order_no);
      $('#dip_adm_order_no').addClass('has-error');
      //return false;
      }
      else
      {
      error_dip_adm_order_no = '';
      $('#error_dip_adm_order_no').text(error_dip_adm_order_no);
      $('#dip_adm_order_no').removeClass('has-error');
      }
      if($.trim($('#dip_dcet_no').val()).length == 0)
      {
      //alert("DCET Number is required");
      error_dip_dcet_no = 'DCET is required.';
      $('#error_dip_dcet_no').text(error_dip_dcet_no);
      $('#dip_dcet_no').addClass('has-error');
      //return false;
      }
      else
      {
        error_dip_dcet_no = '';
      $('#error_dip_dcet_no').text(error_dip_dcet_no);
      $('#dip_dcet_no').removeClass('has-error');
      }

      if($.trim($('#dip_rank').val()).length == 0)
      {
      //alert("DCET Rank is required");
      error_dip_rank = 'DCET Rank is required.';
      $('#error_dip_rank').text(error_dip_rank);
      $('#dip_rank').addClass('has-error');
      //return false;
      }
      else
      {
        error_dip_rank= '';
      $('#error_dip_rank').text(error_dip_rank);
      $('#dip_rank').removeClass('has-error');
      }
      if($.trim($('#dip_cat_claimed').val()).length == 0)
      {
      //alert("Claimed Category is required");
      error_dip_cat_claimed= 'Claimed Category is required';
      $('#error_dip_cat_claimed').text(error_dip_cat_claimed);
      $('#dip_cat_claimed').addClass('has-error');
      //return false;
      }
      else
      {
      ...
      if($.trim($('#dip_cat_allotted').val()).length == 0)
      {
      //alert("Allotted Category is required");
      error_dip_cat_allotted = 'Allotted Category is required';
      $('#error_dip_cat_allotted').text(error_dip_cat_allotted);
      $('#dip_cat_allotted').addClass('has-error');
      //return false;
      }
      else
      {
      ...
      $('#error_dip_cat_allotted').text(error_dip_cat_allotted);
      $('#dip_cat_allotted').removeClass('has-error');
      }
      if($.trim($('#dip_allot_date').val()).length == 0)
      {
      //alert("Allotment Date is required");
      error_dip_allot_date = 'Allotment Date is required';
      $('#error_dip_allot_date').text(error_dip_allot_date);
      $('#dip_allot_date').addClass('has-error');
      //return false;
      }
      else if ($.trim($('#dip_allot_date').val()) > admDate) {
        error_dip_allot_date = 'Valid Allotment Date is required';
        $('#error_dip_allot_date').text(error_dip_allot_date);
        $('#dip_allot_date').addClass('has-error');
        //return false;
      }
      ...
      $('#error_dip_allot_date').text(error_dip_allot_date);
      $('#dip_allot_date').removeClass('has-error');
      }

      if($.trim($('#dip_fees_paid').val()).length == 0)
      {
      //alert("Fees paid at KEA is required");
      error_dip_fees_paid = 'Fees paid at COMEDK is required';
      $('#error_dip_fees_paid').text(error_dip_fees_paid);
      $('#dip_fees_paid').addClass('has-error');
      //return false;
      }
      else
      {
        error_dip_fees_paid = '';
      $('#error_dip_fees_paid').text(error_dip_fees_paid);
      $('#dip_fees_paid').removeClass('has-error');
      }

      if($.trim($('#dip_college_fees_paid').val()).length == 0)
      {
      //alert("Fees paid at college is required");
      error_dip_college_fees_paid = 'Fees paid at college is required';
      $('#error_dip_college_fees_paid').text(error_dip_college_fees_paid);
      $('#dip_college_fees_paid').addClass('has-error');
      //return false;
      }
      else
      {
      ...
      $('#error_dip_college_fees_paid').text(error_dip_college_fees_paid);
      $('#dip_college_fees_paid').removeClass('has-error');
      }
      ...
      error_dip_total_fees_paid = 'Fees paid at college is required';
      $('#error_dip_total_fees_paid').text(error_dip_total_fees_paid);
      $('#dip_total_fees_paid').addClass('has-error');
      //return false;
      }
      else
      {
        error_dip_total_fees_paid = '';
      $('#error_dip_total_fees_paid').text(error_dip_total_fees_paid);
      $('#dip_total_fees_paid').removeClass('has-error');
      }

      if($.trim($('#dip_challan_date').val()).length == 0)
      {
      error_dip_challan_date = 'Challan Date is required';
      //alert(error_dip_challan_date);
      $('#error_dip_challan_date').text(error_dip_challan_date);
      $('#dip_challan_date').addClass('has-error');
      //return false;
      }
      else if ($.trim($('#dip_challan_date').val()) > admDate) {
        error_dip_challan_date = 'Valid Challan Date is required';
        //alert(error_dip_challan_date);
        $('#error_dip_challan_date').text(error_dip_challan_date);
        $('#dip_challan_date').addClass('has-error');
        //return false;
      ...
      {
        error_dip_challan_date = '';
      $('#error_dip_challan_date').text(error_dip_challan_date);
      $('#dip_challan_date').removeClass('has-error');
      }
  ...
  /*  if(error_cet_order_no != '' || error_cet_no != '' || error_cet_rank != '' || error_cet_cat_claimed != '' || error_cet_cat_allotted != '' || error_cet_allot_date != '' || error_cet_kea_fees_paid != '' || error_cet_college_fees_paid != '' || error_cet_total_fees_paid != '' || error_cet_challan_date != '' || error_cet_challan_no != ''){
    ...
    if(error_dip_adm_order_no != '' || error_dip_dcet_no != '' || error_dip_rank  != '' || error_dip_cat_claimed != '' || error_dip_cat_allotted != '' || error_dip_allot_date != '' || error_dip_fees_paid != '' || error_dip_college_fees_paid != '' || error_dip_total_fees_paid != ''|| error_dip_challan_date != '' || error_dip_challan_no != ''){
      return false;    
    }
